...
| `remote_server` | 相手側サーバーの問題(接続拒否・応答なし・サーバーダウン等) | excluded |
...
相手側メールサーバーが接続を拒否、または応答しないケースで、送信側のブロックリスト登録が原因でないもの。送信側では対応不可能なためexcludedに振り分ける。

- "refused to talk to me" + "Access Denied" (ブロックリストの言及なし) → `remote_server`
- リモートサーバーのダウン・応答なし → `remote_server`
...
| `--cleanup [DATE]` | 指定日のレポートJSONとキャッシュエントリを削除して終了(日付省略時: 今日) | - |
| `--report [DATE]` | 指定日のレポートを表示して終了(日付省略時: 今日) | - |
| `--category CATS` | `--report`で表示するカテゴリをカンマ区切りで指定 | target系カテゴリ |
| `--version` | バージョン表示 | - |
...
指定された日付のレポートファイルとキャッシュエントリを削除し、通常のIMAP処理は実行しない。日付省略時は今日を対象とする。
...
**--category**: カンマ区切りでカテゴリを指定。省略時は `excluded: false` のtarget系カテゴリのみ表示。
...
**通常実行時の案内**: target系レコードが検出された場合、サマリー末尾に `--report` コマンドの案内を表示する。
